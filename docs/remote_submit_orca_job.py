...
def write_scheduler_script(running_dir: Path, job_spec: dict[str, Any]) -> Path:
    ...
    script_path.write_text(
        ...
        "TMP_ROOT=\"${TMPDIR:?TMPDIR must be set by the scheduler on the compute node}\"\n"
        "SCRATCH_DIR=\"${TMP_ROOT}/orca_${SLURM_JOB_ID}\"\n"
        "mkdir -p \"${SCRATCH_DIR}\"\n\n"
        "echo \"Submit dir: ${SLURM_SUBMIT_DIR}\" | tee -a run.log\n"
        "echo \"Compute-node temp root: ${TMP_ROOT}\" | tee -a run.log\n"
        "echo \"Scratch dir: ${SCRATCH_DIR}\" | tee -a run.log\n"
        "echo \"ORCA binary: ${ORCA_BIN}\" | tee -a run.log\n\n"
        "cp -f \"${SLURM_SUBMIT_DIR}/job.inp\" \"${SCRATCH_DIR}/job.inp\"\n"
        "cp -f \"${SLURM_SUBMIT_DIR}/input_structure.xyz\" \"${SCRATCH_DIR}/input_structure.xyz\"\n\n"
        "cd \"${SCRATCH_DIR}\"\n"
